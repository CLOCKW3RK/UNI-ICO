...
set /p YuzuDir=Enter where Yuzu is installed... 
set /p GameDir=Enter your game directory, i.e., the FOLDER the game is in... 
set /p GameName=Enter the FULL FILENAME of your NSP/XCI file... 
mkdir "%YuzuDir%\$Scripts"
mkdir "%YuzuDir%\$Scripts\%Name%"
echo Making BAT...
(
  echo @echo off
  echo start /b killcmd
  echo cd "%YuzuDir%"
  echo yuzu "%GameDir%\%GameName%"
) > "%YuzuDir%\$Scripts\%Name%\%Name%.bat"

(
  echo @echo off
  echo timeout /t 3
  echo taskkill /f /im cmd.exe
) > "%YuzuDir%\$Scripts\%Name%\killcmd.bat"
echo Done!
goto nocmd
...
move %Name%.vbs "%YuzuDir%\$Scripts\%Name%"
echo Done!
cls
goto Icon
...
cd "%YuzuDir%\$Scripts\%Name%\"
echo Set oWS = WScript.CreateObject("WScript.Shell") > CreateShortcut.vbs
echo sLinkFile = "%userprofile%\Desktop\%shname%.lnk" >> CreateShortcut.vbs
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> CreateShortcut.vbs
echo oLink.TargetPath = "%YuzuDir%\$Scripts\%Name%\%Name%.vbs" >> CreateShortcut.vbs
echo oLink.WorkingDirectory = "%YuzuDir%\$Scripts\%Name%" >> CreateShortcut.vbs
echo oLink.Description = "%shname%" >> CreateShortcut.vbs
echo oLink.IconLocation = "%YuzuDir%\$Scripts\%Name%\%Name%.ico" >> CreateShortcut.vbs
...
echo sLinkFile = "%userprofile%\Desktop\%shname%.lnk" >> CreateShortcut.vbs
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> CreateShortcut.vbs
echo oLink.TargetPath = "%YuzuDir%\$Scripts\%Name%\%Name%.bat" >> CreateShortcut.vbs
echo oLink.WorkingDirectory = "%YuzuDir%\$Scripts\%Name%" >> CreateShortcut.vbs
echo oLink.Description = "%shname%" >> CreateShortcut.vbs
echo oLink.IconLocation = "%YuzuDir%\$Scripts\%Name%\%Name%.ico" >> CreateShortcut.vbs
echo oLink.Save >> CreateShortcut.vbs
