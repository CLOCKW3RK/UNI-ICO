...
set /p CitraDir=Enter where Citra is installed... 
set /p GameDir=Enter your game directory, i.e., the FOLDER the game is in... 
set /p GameName=Enter the FULL FILENAME of your 3DS file... 
mkdir "%CitraDir%\$Scripts"
mkdir "%CitraDir%\$Scripts\%Name%"
echo Making BAT...
(
  echo @echo off
  echo start /b killcmd
  echo cd "%CitraDir%"
  echo citra-qt "%GameDir%\%GameName%"
) > "%CitraDir%\$Scripts\%Name%\%Name%.bat"

(
  echo @echo off
  echo timeout /t 3
  echo taskkill /f /im cmd.exe
) > "%CitraDir%\$Scripts\%Name%\killcmd.bat"
echo Done!
goto nocmd
...
move %Name%.vbs "%CitraDir%\$Scripts\%Name%"
echo Done!
cls
goto Icon
...
:Icon
explorer "%CitraDir%\$Scripts\%Name%"
...
cd "%CitraDir%\$Scripts\%Name%\"
echo Set oWS = WScript.CreateObject("WScript.Shell") > CreateShortcut.vbs
echo sLinkFile = "%userprofile%\Desktop\%shname%.lnk" >> CreateShortcut.vbs
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> CreateShortcut.vbs
echo oLink.TargetPath = "%CitraDir%\$Scripts\%Name%\%Name%.vbs" >> CreateShortcut.vbs
echo oLink.WorkingDirectory = "%CitraDir%\$Scripts\%Name%" >> CreateShortcut.vbs
echo oLink.Description = "%shname%" >> CreateShortcut.vbs
echo oLink.IconLocation = "%CitraDir%\$Scripts\%Name%\%Name%.ico" >> CreateShortcut.vbs
...
echo sLinkFile = "%userprofile%\Desktop\%shname%.lnk" >> CreateShortcut.vbs
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> CreateShortcut.vbs
echo oLink.TargetPath = "%CitraDir%\$Scripts\%Name%\%Name%.bat" >> CreateShortcut.vbs
echo oLink.WorkingDirectory = "%CitraDir%\$Scripts\%Name%" >> CreateShortcut.vbs
echo oLink.Description = "%shname%" >> CreateShortcut.vbs
echo oLink.IconLocation = "%CitraDir%\$Scripts\%Name%\%Name%.ico" >> CreateShortcut.vbs
echo oLink.Save >> CreateShortcut.vbs
