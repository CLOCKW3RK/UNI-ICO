...
set /p RPCS3Dir=Enter where RPCS3 is installed... 
set /p GameDir=Enter your game directory, i.e., the FOLDER the game is in... 
set /p GameName=Enter the FULL FILENAME of your BIN/ISO file... 
mkdir "%RPCS3Dir%\$Scripts"
mkdir "%RPCS3Dir%\$Scripts\%Name%"
echo Making BAT...
(
  echo @echo off
  echo start /b killcmd
  echo cd "%RPCS3Dir%"
  echo RPCS3 "%GameDir%\%GameName%"
) > "%RPCS3Dir%\$Scripts\%Name%\%Name%.bat"

(
  echo @echo off
  echo timeout /t 3
  echo taskkill /f /im cmd.exe
) > "%RPCS3Dir%\$Scripts\%Name%\killcmd.bat"
echo Done!
goto nocmd
...
move %Name%.vbs "%RPCS3Dir%\$Scripts\%Name%"
echo Done!
cls
goto Icon
...
:Icon
explorer "%RPCS3Dir%\$Scripts\%Name%"
...
cd "%RPCS3Dir%\$Scripts\%Name%\"
echo Set oWS = WScript.CreateObject("WScript.Shell") > CreateShortcut.vbs
echo sLinkFile = "%userprofile%\Desktop\%shname%.lnk" >> CreateShortcut.vbs
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> CreateShortcut.vbs
echo oLink.TargetPath = "%RPCS3Dir%\$Scripts\%Name%\%Name%.vbs" >> CreateShortcut.vbs
echo oLink.WorkingDirectory = "%RPCS3Dir%\$Scripts\%Name%" >> CreateShortcut.vbs
echo oLink.Description = "%shname%" >> CreateShortcut.vbs
echo oLink.IconLocation = "%RPCS3Dir%\$Scripts\%Name%\%Name%.ico" >> CreateShortcut.vbs
...
echo sLinkFile = "%userprofile%\Desktop\%shname%.lnk" >> CreateShortcut.vbs
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> CreateShortcut.vbs
echo oLink.TargetPath = "%RPCS3Dir%\$Scripts\%Name%\%Name%.bat" >> CreateShortcut.vbs
echo oLink.WorkingDirectory = "%RPCS3Dir%\$Scripts\%Name%" >> CreateShortcut.vbs
echo oLink.Description = "%shname%" >> CreateShortcut.vbs
echo oLink.IconLocation = "%RPCS3Dir%\$Scripts\%Name%\%Name%.ico" >> CreateShortcut.vbs
echo oLink.Save >> CreateShortcut.vbs
