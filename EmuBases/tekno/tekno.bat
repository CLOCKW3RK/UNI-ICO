...
set /p TeknoparrotDir=Enter where Teknoparrot is installed... 
set /p GameName=Enter the FULL Profile name... 
mkdir "%TeknoparrotDir%\$Scripts"
mkdir "%TeknoparrotDir%\$Scripts\%Name%"
echo Making BAT...
(
  echo @echo off
  echo start /b killcmd
  echo cd "%TeknoparrotDir%"
  echo TeknoparrotUI.exe "--profile=%GameName%"
) > "%TeknoparrotDir%\$Scripts\%Name%\%Name%.bat"

(
  echo @echo off
  echo timeout /t 3
  echo taskkill /f /im cmd.exe
) > "%TeknoparrotDir%\$Scripts\%Name%\killcmd.bat"
echo Done!
goto nocmd
...
move %Name%.vbs "%TeknoparrotDir%\$Scripts\%Name%"
echo Done!
cls
goto Icon
...
:Icon
explorer "%TeknoparrotDir%\$Scripts\%Name%"
...
cd "%TeknoparrotDir%\$Scripts\%Name%\"
echo Set oWS = WScript.CreateObject("WScript.Shell") > CreateShortcut.vbs
echo sLinkFile = "%userprofile%\Desktop\%shname%.lnk" >> CreateShortcut.vbs
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> CreateShortcut.vbs
echo oLink.TargetPath = "%TeknoparrotDir%\$Scripts\%Name%\%Name%.vbs" >> CreateShortcut.vbs
echo oLink.WorkingDirectory = "%TeknoparrotDir%\$Scripts\%Name%" >> CreateShortcut.vbs
echo oLink.Description = "%shname%" >> CreateShortcut.vbs
echo oLink.IconLocation = "%TeknoparrotDir%\$Scripts\%Name%\%Name%.ico" >> CreateShortcut.vbs
...
echo sLinkFile = "%userprofile%\Desktop\%shname%.lnk" >> CreateShortcut.vbs
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> CreateShortcut.vbs
echo oLink.TargetPath = "%TeknoparrotDir%\$Scripts\%Name%\%Name%.bat" >> CreateShortcut.vbs
echo oLink.WorkingDirectory = "%TeknoparrotDir%\$Scripts\%Name%" >> CreateShortcut.vbs
echo oLink.Description = "%shname%" >> CreateShortcut.vbs
echo oLink.IconLocation = "%TeknoparrotDir%\$Scripts\%Name%\%Name%.ico" >> CreateShortcut.vbs
echo oLink.Save >> CreateShortcut.vbs
