...
set /p CemuDir=Enter where Cemu is installed... 
set /p GameDir=Enter your game directory, i.e., the FOLDER the game is in... 
set /p GameName=Enter the FULL FILENAME of your WUD/RPX file... 
mkdir "%CemuDir%\$Scripts"
mkdir "%CemuDir%\$Scripts\%Name%"
echo Making BAT...
(
  echo @echo off
  echo start /b killcmd
  echo cd "%CemuDir%"
  echo Cemu -f -g "%GameDir%\%GameName%"
) > "%CemuDir%\$Scripts\%Name%\%Name%.bat"

(
  echo @echo off
  echo timeout /t 3
  echo taskkill /f /im cmd.exe
) > "%CemuDir%\$Scripts\%Name%\killcmd.bat"
echo Done!
goto nocmd
...
move %Name%.vbs "%CemuDir%\$Scripts\%Name%"
echo Done!
cls
goto Icon
...
cd "%CemuDir%\$Scripts\%Name%\"
echo Set oWS = WScript.CreateObject("WScript.Shell") > CreateShortcut.vbs
echo sLinkFile = "%userprofile%\Desktop\%shname%.lnk" >> CreateShortcut.vbs
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> CreateShortcut.vbs
echo oLink.TargetPath = "%CemuDir%\$Scripts\%Name%\%Name%.vbs" >> CreateShortcut.vbs
echo oLink.WorkingDirectory = "%CemuDir%\$Scripts\%Name%" >> CreateShortcut.vbs
echo oLink.Description = "%shname%" >> CreateShortcut.vbs
echo oLink.IconLocation = "%CemuDir%\$Scripts\%Name%\%Name%.ico" >> CreateShortcut.vbs
...
echo sLinkFile = "%userprofile%\Desktop\%shname%.lnk" >> CreateShortcut.vbs
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> CreateShortcut.vbs
echo oLink.TargetPath = "%CemuDir%\$Scripts\%Name%\%Name%.bat" >> CreateShortcut.vbs
echo oLink.WorkingDirectory = "%CemuDir%\$Scripts\%Name%" >> CreateShortcut.vbs
echo oLink.Description = "%shname%" >> CreateShortcut.vbs
echo oLink.IconLocation = "%CemuDir%\$Scripts\%Name%\%Name%.ico" >> CreateShortcut.vbs
echo oLink.Save >> CreateShortcut.vbs
